...
export default function Discovery({ session }) {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8">
      <section className="overflow-hidden rounded-3xl bg-gradient-to-br from-primary-900 via-primary-700 to-primary-400 px-6 py-10 text-white shadow-lg sm:px-10">
        <div className="max-w-3xl">
          ...
          <label className="mt-7 flex max-w-2xl items-center gap-3 rounded-xl bg-white p-2 pl-4 shadow-xl shadow-primary-900/20" htmlFor="program-search">
      ...
      <section aria-labelledby="destinations-title">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {result.featuredCountries.map((item) => (
            <button className={`group rounded-2xl border bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${country === item.name ? 'border-primary-500 ring-2 ring-primary-100' : 'border-secondary-200 hover:border-primary-200'}`} key={item.id} onClick={() => setCountry(country === item.name ? '' : item.name)} type="button">
              <div className="flex items-start justify-between"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-primary-50 text-lg font-bold text-primary-600">{item.code}</div><span aria-hidden="true">→</span></div>
      ...
      <section aria-labelledby="programs-title">
        <div className="mb-4 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div><p className="text-sm font-semibold text-primary-600">Curated for students</p><h2 className="mt-1 text-2xl font-bold text-secondary-950" id="programs-title">Featured programs</h2><p className="mt-1 text-sm text-secondary-500">{loading ? 'Finding programs…' : `${result.totalCount} program${result.totalCount === 1 ? '' : 's'} found`}</p></div>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            {result.programs.map((item) => (
              <article className="rounded-2xl border border-secondary-200 bg-white p-5 shadow-sm" key={item.id}>
                <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wide text-primary-600">{item.country} · {item.city}</p><h3 className="mt-2 text-lg font-bold text-secondary-950">{item.name}</h3><p className="mt-1 text-sm text-secondary-600">{item.university}</p></div><div className="flex shrink-0 items-center gap-2"><span className="rounded-full bg-primary-50 px-3 py-1 text-xs font-bold text-primary-600">{item.matchScore}% match</span><button aria-label={`${item.isSaved ? 'Remove' : 'Add'} ${item.name} ${item.isSaved ? 'from' : 'to'} shortlist`} className={`flex h-8 w-8 items-center justify-center rounded-full border text-sm transition disabled:opacity-50 ${item.isSaved ? 'border-primary-500 bg-primary-500 text-white' : 'border-secondary-200 text-secondary-400 hover:border-primary-300 hover:text-primary-600'}`} disabled={savingIds.has(item.id)} onClick={() => toggleSaved(item)} type="button">{item.isSaved ? '♥' : '♡'}</button></div></div>
                <div className="mt-5 flex flex-wrap gap-2">{item.tags.map((tag) => <span className="rounded-md bg-secondary-100 px-2.5 py-1 text-xs font-medium text-secondary-600" key={tag}>{tag}</span>)}</div>
